...
First, look at measures of spread for home prices to get an idea of what the bins should be. Mean price is larger than median price, indicating that this variable is skewed right.
...
Property Price by Region: The metropolitan areas have the widest ranges of home prices, whereas there are several price ranges with no properties in Victoria. The southern metropolitan area looks to be the most expensive region.
...
Average and median property price by year sold over the last 50 years
...
Histogram of home prices, with and without outliers & Bar chart of price range
...
Line plot of average Price vs Year Built
...
Price Range by Year Sold

```{r}
ggplot(df, aes(x = PriceRange, fill = YearSold)) +
  geom_bar(position = 'dodge') +
  scale_fill_brewer(palette = 'Paired') +
  labs(x = 'Price Range',
       title = 'Price Range by Year Sold')
```
